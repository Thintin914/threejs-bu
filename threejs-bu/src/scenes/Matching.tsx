    ...
    useEffect(() => {
        ...
        room.current
            ...
            .subscribe(async (status) => {
                ...
                // initial
                setSkin('knight1runS.glb');
                await room.current!.track({
                    is_host: is_host,
                    username: account.username,
                    skin: 'knight1runS.glb'
                });
            });
            ...
            <div className=' absolute z-20 w-full flex bottom-0 left-0 p-2 gap-4 whitespace-break-spaces'>
                <motion.div className=" pointer-events-auto text-sm font-semibold p-1 pl-2 pr-2 border-2 border-white rounded-md select-none text-white cursor-pointer"
                    ...
                    onClick={async () => {
                        ...
                        if (!player)
                            return;
                        if (player.skin === 'knight4runS.glb')
                            return;
                        setSkin('knight4run.glb')
                        await room.current!.track({
                            is_host: is_host,
                            username: account.username,
                            skin: 'knight4runS.glb'
                        });
                    }}>
                    Tint
                </motion.div>
                <motion.div className=" pointer-events-auto text-sm font-semibold p-1 pl-2 pr-2 border-2 border-white rounded-md select-none text-white cursor-pointer"
                    ...
                    onClick={async () => {
                        ...
                        if (player.skin === 'knight3run.glb')
                            return;
                        setSkin('knight3runS.glb')
                        await room.current!.track({
                            is_host: is_host,
                            username: account.username,
                            skin: 'knight3runS.glb'
                        });
                    }}>
                    Blue
                </motion.div>
                <motion.div className=" pointer-events-auto text-sm font-semibold p-1 pl-2 pr-2 border-2 border-white rounded-md select-none text-white cursor-pointer"
                    ...
                    onClick={async () => {
                        ...
                        if (!player)
                            return;
                        if (player.skin === 'knight2runS.glb')
                            return;
                        setSkin('knight2runS.glb')
                        await room.current!.track({
                            is_host: is_host,
                            username: account.username,
                            skin: 'knight2runS.glb'
                        });
                    }}>
                    Yellow
                </motion.div>
                <motion.div className=" pointer-events-auto text-sm font-semibold p-1 pl-2 pr-2 border-2 border-white rounded-md select-none text-white cursor-pointer"
                    ...
                    onClick={async () => {
                        ...
                        if (!player)
                            return;
                        if (player.skin === 'knight1runS.glb')
                            return;
                        setSkin('knight1runS.glb')
                        await room.current!.track({
                            is_host: is_host,
